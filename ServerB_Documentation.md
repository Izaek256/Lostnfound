# ServerB Documentation - User Logic & Database Server

## 🎯 Server Role & Responsibility

**ServerB** is the **User Logic & Database Server** in the Lost & Found distributed system. It serves as the **central hub** for user authentication, database hosting, file storage, and acts as a proxy for item-related operations.
...
### What ServerB Does NOT Handle
❌ Direct item CRUD operations (delegated to ServerA)  
...
- **Base URL**: `http://172.24.194.6/Lostnfound/ServerB`
- **API Base URL**: `http://172.24.194.6/Lostnfound/ServerB/api`
- **Location**: Same machine as ServerA
...
**Usage Example:**
```php
// Call ServerA to get user's items
$response = makeAPIRequest(
    SERVERA_API_URL . '/get_user_items.php',
    ['user_id' => $user_id],
    'GET',
...
**Note**: These are primarily used by ServerC (UI). ServerB APIs work independently.
...
**Purpose**: Proxy requests to ServerA to get items posted by a specific user.
...
{
  "error": "Unexpected response from ServerA"
}
```

**Proxy Implementation:**
This endpoint demonstrates ServerB's **proxy pattern** for item operations:

```php
// Build ServerA URL
$servera_url = SERVERA_API_URL . '/get_user_items.php';

// Make API call to ServerA
$response = makeAPIRequest(
    $servera_url,
    ['user_id' => $user_id],
    'GET',
...
// Forward ServerA's response to client
if (is_array($response) && isset($response['success'])) {
...
**Why Use a Proxy?**
- **Centralized access control**: ServerB can add authentication checks
- **Request logging**: All item requests go through ServerB
- **Consistent API**: Clients only need to know ServerB's URL
...
**Registration Flow:**
```
User fills registration form on ServerC
    ↓
ServerC calls ServerB: POST /register_user.php
    ↓
ServerB validates email format
    ↓
ServerB checks for existing username/email
    ↓
ServerB hashes password
    ↓
ServerB inserts into database
    ↓
ServerB returns user data + success
    ↓
ServerC creates session and logs user in
```

**Login Flow:**
```
User enters credentials on ServerC
    ↓
ServerC calls ServerB: POST /verify_user.php
    ↓
ServerB queries user by username
    ↓
ServerB verifies password using password_verify()
    ↓
ServerB returns user data (including is_admin flag)
    ↓
ServerC stores user data in session
    ↓
ServerC redirects to dashboard
```

### ServerB → ServerA (Item Proxy)

**Get User Items Flow:**
```
Admin views user dashboard on ServerC
    ↓
ServerC calls ServerB: GET /get_user_items.php?user_id=5
    ↓
ServerB proxies to ServerA: GET /get_user_items.php?user_id=5
    ↓
ServerA queries items table
    ↓
ServerA returns items + statistics
    ↓
ServerB forwards response to ServerC
    ↓
ServerC displays items to user
```

**Why Proxy Through ServerB?**
1. **Centralized authentication**: Can add auth checks before proxying
2. **Consistent client interface**: ServerC only talks to ServerB for user-related operations
...
ServerB uses auto-generated deployment configuration from `deployment_config.php`.
...
## 🧪 Testing ServerB APIs

### Using cURL

**Test User Registration:**
```bash
curl -X POST http://172.24.194.6/Lostnfound/ServerB/api/register_user.php \
  -d "username=testuser" \
...
**Test User Login:**
```bash
curl -X POST http://172.24.194.6/Lostnfound/ServerB/api/verify_user.php \
  -d "username=testuser" \
  -d "password=testpass123"
```
...
**Test Get All Users:**
```bash
curl http://172.24.194.6/Lostnfound/ServerB/api/get_all_users.php
```

**Test Get User Items (Proxy):**
```bash
curl http://172.24.194.6/Lostnfound/ServerB/api/get_user_items.php?user_id=1
```

**Test Toggle Admin:**
```bash
curl -X POST http://172.24.194.6/Lostnfound/ServerB/api/toggle_admin.php \
  -d "user_id=5" \
  -d "is_admin=1"
```
...
**Test Health Check:**
```bash
curl http://172.24.194.6/Lostnfound/ServerB/api/health.php
```
...
**Solution**: Check ServerA health endpoint, verify network connectivity
...
**ServerB serves as the central hub for user authentication, database management, and inter-server communication in the Lost & Found system.**
