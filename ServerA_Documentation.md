# ServerA Documentation - Item Logic Server

## 🎯 Server Role & Responsibility

**ServerA** is the **Item Logic Server** in the Lost & Found distributed system. It serves as the **sole authority** for all item-related operations, including creating, reading, updating, and deleting items in the database.
...
- **Base URL**: `http://172.24.194.6/Lostnfound/ServerA`
- **API Base URL**: `http://172.24.194.6/Lostnfound/ServerA/api`
- **Location**: Same machine as ServerB
- **Role**: Item Logic Server

### Database Connection
- **Host**: `172.24.194.6` (Remote connection to ServerB's database)
...
**Note**: While these functions exist in ServerA's config, they are primarily used by ServerC (UI server). ServerA APIs work independently of sessions.
...
### ServerB → ServerA
**Use Case**: When ServerB needs item data for a user dashboard

**Flow:**
```
ServerB receives request from client
    ↓
ServerB calls ServerA: GET /get_user_items.php?user_id=5
    ↓
ServerA queries database
    ↓
ServerA returns JSON with items
    ↓
ServerB forwards to client
```

**Implementation** (in ServerB's `get_user_items.php`):
```php
...
ServerC calls ServerA: POST /add_item.php
    ↓
ServerA validates and inserts into database
    ↓
ServerA returns success/error JSON
    ↓
ServerC displays result to user
```
...
ServerA uses auto-generated deployment configuration from `deployment_config.php`.
...
## 🧪 Testing ServerA APIs

### Using cURL

**Test Add Item:**
```bash
curl -X POST http://172.24.194.6/Lostnfound/ServerA/api/add_item.php \
  -d "user_id=1" \
  -d "title=Test Item" \
...
**Test Get All Items:**
```bash
curl http://172.24.194.6/Lostnfound/ServerA/api/get_all_items.php
```

**Test Search:**
```bash
curl "http://172.24.194.6/Lostnfound/ServerA/api/get_all_items.php?search=iPhone"
```

**Test Health Check:**
```bash
curl http://172.24.194.6/Lostnfound/ServerA/api/health.php
```
...
**Cause**: Cannot connect to remote database on ServerB  
**Solution**: 
- Check ServerB MySQL is running
...
**ServerA serves as the backbone of item management in the Lost & Found system, providing reliable and secure API endpoints for all item-related operations.**
